...
public class Mp3File extends FileWrapper {
	...
	private void init(int bufferLength, boolean scanFile) throws IOException, UnsupportedTagException, InvalidDataException {
		...
		inFile = new RandomAccessFile(mFile,"r");
	...
	protected int preScanFile() {
		ByteBuffer byteBuffer = ByteBuffer.allocate(AbstractID3v2Tag.HEADER_LENGTH);
		try {
			inFile.seek(0);
			byteBuffer.clear();
			int bytesRead = inFile.read(byteBuffer.array());
			if (bytesRead == AbstractID3v2Tag.HEADER_LENGTH) {
				try {
					byte[] bytes = byteBuffer.array();
					ID3v2TagFactory.sanityCheckTag(bytes);
	...
	private void scanFile() throws IOException, InvalidDataException {
		ByteBuffer byteBuffer = ByteBuffer.allocate(bufferLength);
		//byte[] buffer = new byte[bufferLength];
		//byteBuffer.array()[0] = buffer[0];
		
		int fileOffset = preScanFile();
	...
	private void initId3v1Tag() throws IOException {
		//ByteBuffer byteBuffer = ByteBuffer.allocate(ID3v1Tag.TAG_LENGTH);
		byte[] buff = new byte[ID3v1Tag.TAG_LENGTH];
		
		RandomAccessFile in = new RandomAccessFile(mFile,"r");
		in.seek(getLength() - ID3v1Tag.TAG_LENGTH);
		//byteBuffer.clear();
		int bytesRead = in.read(buff);
		if (bytesRead < ID3v1Tag.TAG_LENGTH) throw new IOException("Not enough bytes read");
		try {
			id3v1Tag = new ID3v1Tag(buff);
		} catch (NoSuchTagException e) {
			id3v1Tag = null;
		}
	}
